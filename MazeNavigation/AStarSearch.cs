...
namespace MazeNavigation
{
    class AStarSearch
    {
        ...
        public AStarSearch(int n, int m)
        {
            searched = 0;
            discovered = 0;
        ...
        public void AS(RobotNavGrid<char[,]> grid, Agent agent, GoalState goalState, List<GridWall> gridWalls)
        {
            ...
            for (int n = 0; n < N; n++) // rows 
            {
                ...
                {
                    distanceGrid[n, m] = ManhattanDistance(n, m, goalState.X, goalState.Y) + ManhattanDistance(n, m, agent.X, agent.Y);
                }
            }
            ...
            while (true)
            {
                ...
                for (int i = 0; i < 4; i++) // traverses the grid
                {
                    ...
                    if (distanceGrid[adjy, adjx] < smallestDistance || distanceGrid[adjy, adjx] == smallestDistance) // compares the current cell's manhattan heuristic with the smallestDistance value
                    {
                        discovered++;
        ...
        static int ManhattanDistance(int N, int M, int x, int y) // returns the manhattan distance from the goal to the row and colum index (n & m)
        {
